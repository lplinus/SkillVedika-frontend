export async function submitDemoForm(data: any) {
  ...
  if (!enabled) {
    throw new Error(
      'Demo submissions are disabled. Set NEXT_PUBLIC_DEMO_SUBMISSIONS_ENABLED=true to enable.'
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to submit demo form: ${response.status} ${response.statusText} ${text || ''}`
      );
    }
