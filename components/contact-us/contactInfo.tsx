...
export default function ContactInfo({
  ...
  const titlePart1 = hasParts
    ? (title?.part1 ?? title?.text ?? '')
    : (title?.text ?? 'Get in Touch');
  const titlePart2 = hasParts ? (title?.part2 ?? '') : '';

  return (
<section className="relative overflow-hidden bg-background py-12 px-4 sm:px-6 lg:py-24 lg:px-8">
  <div className="max-w-7xl mx-auto relative z-10">
    {/* Header */}
    <div className="text-center mb-10 md:mb-16 lg:mb-20 space-y-4 md:space-y-5 animate-fade-in-up">
      {/* Target label */}
      <div className="inline-block px-4 py-1.5 md:px-5 md:py-2 bg-primary/10 text-primary font-semibold rounded-full text-xs md:text-sm tracking-wide shadow-sm">
    ...
    {/* Contact Cards Grid */}
    {/* Switched to 1 col on mobile, 2 cols on medium+ */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-10">
      {/* Email */}
      <AnimatedCard
        icon={<Mail className="w-5 h-5 md:w-6 md:h-6 text-black" />}
        label={emailLabel || 'EMAIL US'}
        text={
          <a
            href={emailLink || `mailto:${emailId}`}
            className="text-foreground font-semibold text-base sm:text-lg leading-relaxed hover:underline break-all md:break-normal"
          >
            {emailId}
          </a>
        }
      ...
      {/* Phone */}
      <AnimatedCard
        icon={<Phone className="w-5 h-5 md:w-6 md:h-6 text-black" />}
        label={phoneLabel || 'CALL US'}
        text={
          <a
            href={phoneLink || `tel:${phoneNumber}`}
            className="text-foreground font-semibold text-base sm:text-lg leading-relaxed hover:underline"
          >
            {phoneNumber}
          </a>
        }
      ...
      {/* USA Office */}
      <AnimatedCard
        icon={<MapPin className="w-5 h-5 md:w-6 md:h-6 text-black" />}
